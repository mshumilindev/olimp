...
function AdminPanelStudents({ loading, usersList, classesList }) {
  ...
  function _renderTeacher(student) {
    const selectedClass = classesList
      ? classesList.find((item) => item.id === student.class)
      : null;
    ...
    return (
      <div className="adminDashboard__teachersList-item" key={student.id}>
        <div
          className="adminDashboard__teachersList-avatar"
          style={{ backgroundImage: "url(" + student.avatar + ")" }}
        >
          {!student.avatar ? <i className="fa fa-user" /> : null}
        </div>
        <div className="adminDashboard__teachersList-info">
          <div className="adminDashboard__teachersList-name">
            <Link to={"/admin-users/" + student.login}>{student.name}</Link>
          </div>
