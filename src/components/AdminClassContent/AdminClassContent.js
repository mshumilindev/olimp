...
function AdminClassContent({
  ...
  return (
    <>
      <div className="widget">
        ...
        {
          <CoursesPicker
            selectedCourses={content ? JSON.stringify(content.courses) : []}
      ...
      <div className="widget">
        ...
        {content ? (
          <UserPicker
            type="student"
            noneditable
            selectedList={filterStudents() ? filterStudents() : []}
          />
        ) : loading ? (
          <Preloader />
        ) : null}
    ...
    return (
      <div className="adminClass__schedule">
        {parsedSchedule.map((day) => (
          <AdminClassScheduleDay
            day={day}
            key={day.title}
            selectedCourses={content.courses}
            content={content}
  ...
  function handleAddSchedule(newDay) {
    ...
    newSchedule.find((item) => item.title === newDay.title).lessons =
      newDay.lessons;
