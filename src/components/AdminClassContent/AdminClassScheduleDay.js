...
function AdminClassScheduleDay({
  canEdit,
  day,
  selectedCourses,
  coursesList,
  ...
  if (day.lessons.length) {
    day.lessons.forEach((item) => {
      const currentLesson = selectedCourses.some(
        (course) =>
          course.subject === item.subject && course.course === item.course,
      );
  ...
  return (
    <div className="adminClass__schedule-item">
      ...
      <div className="coursesPicker__selectedList">
        {coursesList && day.lessons.length && coursesList.length
          ? day.lessons
              ...
              .map((lesson, index) => (
                <AdminClassScheduleDayLesson
                  lesson={lesson}
                  index={index}
                  coursesList={coursesList}
                  quickRemoveLesson={quickRemoveLesson}
                  key={index + lesson.course}
                  canEdit={canEdit}
                />
              ))
          : null}
      ...
      {canEdit ? (
        <div
          className="adminClass__schedule-item-add"
    ...
    newSelectedLessons.splice(
      newSelectedLessons.indexOf(
        newSelectedLessons.find(
          (newItem) =>
            newItem.subject === lesson.subject &&
            newItem.course === lesson.course &&
            newItem.time === lesson.time,
        ),
      ),
      1,
    );
  ...
  function onAddLessons(e) {
    ...
    handleAddSchedule({
      ...day,
      lessons: [...day.lessons, ...JSON.parse(selectedLessons)],
    });
