...
export default function AdminClassScheduleDayCourse({
  selectedLessons,
  coursesList,
  item,
  setSelectedLessons,
  ...
  const [time, setTime] = useState({
    start: item.time && item.time.start ? item.time.start : initialTime.start,
    end: item.time && item.time.end ? item.time.end : initialTime.end,
  });
  ...
  return (
    <div
      className={classNames("adminClass__schedule-courses-item", {
        selected: JSON.parse(selectedLessons).some(
          (lesson) =>
            lesson.subject === item.subject && lesson.course === item.course,
        ),
      })}
    >
      <div className="adminClass__schedule-courses-item-inner">
        {JSON.parse(selectedLessons).some(
          (lesson) =>
            lesson.subject === item.subject && lesson.course === item.course,
        ) ? (
          <i
            className="content_title-icon far fa-check-square"
            onClick={toggleLesson}
          />
        ) : (
          <i
            className="content_title-icon far fa-square"
            onClick={toggleLesson}
          />
        )}
        <div
          className="adminClass__schedule-courses-item-subject"
          onClick={toggleLesson}
        >
        ...
        <div
          className="adminClass__schedule-courses-item-course"
          onClick={toggleLesson}
        >
      ...
      <form className="form" style={{ minHeight: 0, width: "auto" }}>
        <div className="form__row">
          ...
          <div className="form__col">&mdash;</div>
          <div className="form__col">
            <input
              type="time"
              className="form__field"
  ...
  function toggleLesson() {
    ...
    if (
      newSelectedLessons.some(
        (newItem) =>
          newItem.subject === item.subject && newItem.course === item.course,
      )
    ) {
      newSelectedLessons.splice(
        newSelectedLessons.indexOf(
          newSelectedLessons.find(
            (newItem) =>
  ...
  function handleTime(type, newTime) {
    const newSelectedLessons = JSON.parse(selectedLessons);
    const currentLesson = newSelectedLessons.find(
      (newItem) =>
        newItem.subject === item.subject && newItem.course === item.course,
    );
    const newTimeItem = {
