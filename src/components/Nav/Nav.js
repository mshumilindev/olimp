...
function Nav({ user, nav, prefix, showLogo, hideItems, logo }) {
  ...
  const [isNavCollapsed, setIsNavCollapsed] = useState(
    localStorage.getItem("isNavCollapsed"),
  );
  ...
  const _renderItem = useCallback(
      ...
      if (
        ...
        return (
          <li key={item.id} className={prefix + "nav_item"}>
            <Link
              to={item.url}
              className={classNames(prefix + "nav_link type-" + item.name, {
                active:
            ...
            >
              {isNavCollapsed ? (
                ...
                <>
                  <i className={prefix + "nav_icon " + item.icon} />
                  <span className={prefix + "nav_text"}>
                    {translate(item.name)}
                  </span>
                </>
              )}
    ...
    [hideItems, isNavCollapsed, prefix, translate, location],
  );
      ...
      <ul className={prefix + "nav_list"}>
        ...
        {user.role === "admin" || user.role === "teacher" ? (
          <li className={prefix + "nav__collapse"}>
            <span
              className={prefix + "nav__collapse-inner"}
              onClick={collapseNav}
            >
              {isNavCollapsed ? (
