...
function AdminLibraryListItem({
  ...
  const [itemFields, setItemFields] = useState(
    getDocFormFields(item.name, item.tags, item.teacher, translate("update")),
  );
  ...
  return (
    ...
    >
      ...
      <td className="table__body-cell">{_renderTeachers()}</td>
      <td className="table__body-cell">
        <div className="table__actions">
          <a
            href="/"
            className="table__actions-btn"
            onClick={(e) => onSetShowModal(e)}
          >
          ...
          <a
            href="/"
            className="table__actions-btn table__actions-btn-error"
        ...
        {showModal ? (
          <>
            <Modal onHideModal={onHideModal}>
              <Form
                heading={translate("edit_doc")}
                fields={itemFields}
                formAction={handleUpdateFile}
  ...
  function _renderTeacherItem(teacher) {
    return getTeacher(teacher) ? (
      <Link to={"/admin-users/" + getTeacher(teacher).login} key={teacher}>
        <i className="fa fa-user content_title-icon" />
  ...
  function setFieldValue(fieldID, value) {
    ...
    if (fieldID === "teacher") {
      newFields.find(
        (newFieldItem) => newFieldItem.id === "teacher_block",
      ).children[0].value = value || "";
    } else {
      newFields.find((newFieldItem) => newFieldItem.id === fieldID).value =
        value;
    }
  ...
  function handleUpdateFile() {
    ...
    newFile.name = itemFields.find(
      (fieldItem) => fieldItem.id === "name",
    ).value;
    newFile.tags = itemFields.find(
      (fieldItem) => fieldItem.id === "tags",
    ).value;
