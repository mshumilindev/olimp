...
class AdminTranslationsList extends React.Component {
          ...
          <>
            <div className="table__holder">
              <table className="adminTranslations__table table">
                <colgroup>
                  {this.cols.map((col) => (
                    <col width={col.width} key={col.id} />
                  ))}
                </colgroup>
                <thead>
                  <tr className="table__head-row">
                    {this.cols.map((col) => (
                      <th className="table__head-cell" key={col.id}>
                        {translate(col.name)}
                      </th>
                    ))}
                  </tr>
                      ...
                      <tr className="table__body-row" key={item.id + index}>
                        ...
                        {item.langs.map((lang) => {
                          return (
                            ...
                            >
                              <AdminTranslationsListItem
                                lang={Object.keys(lang)[0]}
                                item={lang[Object.keys(lang)[0]]}
                                itemKey={item.id}
                              />
                            </td>
                          );
  ...
  notEqual(current, next) {
    ...
    if (isLoaded) {
      if (
        JSON.stringify(current.list) === JSON.stringify(next.list) &&
        current.pager === next.pager
      ) {
        return false;
      }
    }
