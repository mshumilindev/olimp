...
function FiltersSelectClass({filterChanged, classesList, selectedClass}) {
    ...
    function getClassesFields() {
        return [{
            ...
            id: 'selectedClass',
            value: selectedClass ?
                classesList.find(item => item.id === selectedClass).title[lang] ? classesList.find(item => item.id === selectedClass).title[lang] : classesList.find(item => item.id === selectedClass).title['ua']
                :
                '',
            placeholder: translate('classes'),
            options: orderBy(classesList.map(item => { return {id: item.id, title: item.title[lang] ? item.title[lang] : item.title['ua']}}), v => v.title)
        }];
    }
}
