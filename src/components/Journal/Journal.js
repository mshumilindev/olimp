...
const Journal = ({
  ...
  const currentClassSchedule = useMemo(() => {
    return classesList?.find((classItem) => classItem.id === user?.class)
      ?.schedule;
  }, [classesList, user]);
  const { lang, translate } = useContext(siteSettingsContext);
  const startOfWeek = useMemo(
    () => moment().startOf("week").add(shift, "week").unix(),
    [shift],
  );
  ...
  const getLessonTop = useCallback(
    (day, startTime) => {
      const dayStartToUnix = moment(
        `${moment(day * 1000).format("MM/DD/YYYY")} ${time[0]}`,
      ).unix();
      const dayEndToUnix = moment(
        `${moment(day * 1000).format("MM/DD/YYYY")} ${time[time.length - 1]}`,
      ).unix();
      const lessonTimeStartToUnix = moment(
        `${moment(day * 1000).format("MM/DD/YYYY")} ${startTime}`,
      ).unix();
  ...
  const getLessonHeight = useCallback(
    (day, { start, end }) => {
      const dayStartToUnix = moment(
        `${moment(day * 1000).format("MM/DD/YYYY")} ${time[0]}`,
      ).unix();
      ...
      const lessonTimeStartToUnix = moment(
        `${moment(day * 1000).format("MM/DD/YYYY")} ${start}`,
      ).unix();
      const lessonTimeEndToUnix = moment(
        `${moment(day * 1000).format("MM/DD/YYYY")} ${end}`,
      ).unix();
      const dayLength = dayEndToUnix - dayStartToUnix;
  ...
  const getAttendance = useCallback(
    (day, lesson) => {
      ...
      const foundAttItem = foundAttDay.courses.find(
        (course) =>
          course.course === lesson.course && course.start === lesson.time.start,
      );
  ...
  const getIsCurrentLesson = useCallback(
    (day, { start, end }) => {
      const lessonTimeStartToUnix = moment(
        `${moment(day * 1000).format("MM/DD/YYYY")} ${start}`,
      ).unix();
      ...
      const currentDayTimeToUnix = moment(
        `${moment(day * 1000).format("MM/DD/YYYY")} ${currentDayTime}`,
      ).unix();

      return (
        day === today &&
        currentDayTimeToUnix >= lessonTimeStartToUnix &&
        currentDayTimeToUnix <= lessonTimeEndToUnix
      );
    },
      ...
      <div className="journal">
        <div className="journal__days">
          ...
          {week.map((day) => {
            const currentLessons =
              currentClassSchedule?.[moment.unix(day).weekday()]?.lessons;

            return (
              <div
                className={`journal__day${day === today ? " active" : ""}${!currentLessons?.length ? " isHoliday" : ""}`}
                key={day}
              >
                <span>
                ...
                <span className="journal__day-date">
                  <strong>{moment.unix(day).format("DD MMM")}</strong>{" "}
                  {moment.unix(day).format("YYYY")}
                </span>
              </div>
            );
          })}
        ...
        <div className="journal__schedule">
          ...
          <div className="journal__currentTime-holder">
            <div
              className="journal__currentTime"
              style={{ top: getLessonTop(today, currentDayTime) }}
            />
          </div>
          {week.map((day) => {
            const currentLessons =
            ...
            return (
              <div
                className={`journal__lessons${day === today ? " isToday" : ""}`}
                key={day}
              >
                {currentLessons?.map((lesson) => {
                  const currentLessonDateTime = moment(
                    `${moment(day * 1000).format("MM/DD/YYYY")} ${lesson.time.start}`,
                  ).unix();
                  ...
                  const teacher = usersList?.find(
                    (user) => user.id === currentCourse?.teacher,
                  );
                  const { attended, mark } = getAttendance(day, lesson);
                  const timeNow = moment().unix();
                  const lessonTimeEnd = moment(
                    `${moment(day * 1000).format("MM/DD/YYYY")} ${lesson.time.end}`,
                  ).unix();
                  const timePassed = timeNow >= lessonTimeEnd;
                  const chat = events?.participant?.find(
                    (chat) =>
                      chat.datetime === currentLessonDateTime &&
                      chat.organizer === teacher?.id,
                  );

                  return (
                    <div
                      className={`journal__lessons-item${timePassed ? (attended ? " attended" : " skipped") : ""}${getIsCurrentLesson(day, lesson.time) ? " isCurrent" : ""}`}
                      key={`${lesson.testsubject1}_${lesson.time.start}_${lesson.time.end}`}
                      style={{
                        top: getLessonTop(day, lesson.time.start),
                        height: getLessonHeight(day, lesson.time),
                      }}
                    >
                      ...
                      <div className="journal__lessons-name">
                        ...
                        {!!chat?.lesson?.lessonName && (
                          <span>
                            <b>{translate("lesson")}</b>:{" "}
                            <Link
                              to={`/courses/${chat.lesson.subjectID}/${chat.lesson.courseID}/${chat.lesson.moduleID}/${chat.lesson.lessonID}`}
                            >
                              {chat.lesson.lessonName}
                            </Link>
                          </span>
                      ...
                      {attended ? (
                        !!mark ? (
                          <div className="journal__lessons-mark">
                            {`${translate("mark")}: `}
                            <strong>{mark}</strong>
                          </div>
                        ) : null
                      ) : (
                        <div className="journal__lessons-skipped">
                          {timePassed && "Пропуск"}
                        </div>
                      )}
                    </div>
