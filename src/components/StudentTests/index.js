...
const StudentTests = ({showChecked = false, hideTitle = false, hideBtn = false, tests, testsLoading, classData, classDataLoading, allCoursesList}) => {
  const { lang } = useContext(siteSettingsContext);
  const newTests = useMemo(() => {
    return tests?.filter(({ lesson: { courseID } }) => classData?.courses?.find(({course}) => courseID === course)).sort((a, b) => {
      return a.lesson.courseID - b.lesson.courseID
    });
  }, [tests, classData]);

  const testsNotSent = useMemo(() => {
    return newTests?.filter(({blocks, isSent}) => blocks.some(({value}) => !!value.length) && !isSent);
  }, [newTests]);
  ...
  const otherNewTests = useMemo(() => {
    return newTests?.filter(({id, score}) => !testsNotSent?.find((item) => item.id === id) && !testsSentForChecking?.find((item) => item.id === id) && !!score);
  ...
  const getCourseName = useCallback((subjectID, courseID) => {
  ...
  return (
    <StudentTestsStyled>
      ...
      <RowStyled>
              ...
              <div className="studentTests__notSent">
                {
                  testsNotSent.map((test, index) => (
                    <React.Fragment key={test.id}>
                      {
                        checkShowCourseTitle(testsNotSent, index, test.lesson.courseID) && (
                          <StudentTestCourseStyled>
                            { getCourseName(test.lesson.subjectID, test.lesson.courseID) }
                          </StudentTestCourseStyled>
                        )
                      }
        ...
        }
        {
          !!testsSentForChecking?.length && (
            <ColumnStyled>
              <StudentTestsSubtitleStyled>Відправлені на перевірку</StudentTestsSubtitleStyled>
              <div className="studentTests__notSent">
                {
                  testsSentForChecking.map((test, index) => (
                    <React.Fragment key={test.id}>
                      {
                        checkShowCourseTitle(testsSentForChecking, index, test.lesson.courseID) && (
                          <StudentTestCourseStyled>
                            { getCourseName(test.lesson.subjectID, test.lesson.courseID) }
                          </StudentTestCourseStyled>
                        )
                      }
                      <StudentTestItem test={test} icon="fa-regular fa-hourglass-half" />
                    </React.Fragment>
                  ))
                }
              </div>
              ...
              <div className="studentTests__notSent">
                {
                  otherNewTests.map((test, index) => (
                    <React.Fragment key={test.id}>
                      {
                        checkShowCourseTitle(otherNewTests, index, test.lesson.courseID) && (
                          <StudentTestCourseStyled>
                            { getCourseName(test.lesson.subjectID, test.lesson.courseID) }
                          </StudentTestCourseStyled>
                        )
                      }
                      <StudentTestItem test={test} icon="fa-solid fa-check" showScore />
                    </React.Fragment>
                  ))
                }
              </div>
      ...
      }
      {
        !hideBtn && (
          <BtnHolderStyled>
            <Link to={'/tests'} className="btn btn_primary">Усі тести</Link>
          </BtnHolderStyled>
        )
      }
    </StudentTestsStyled>
