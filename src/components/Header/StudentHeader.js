...
const StudentHeader = ({ user, fetchClass, logo, siteName, logoutUser }) => {
  ...
  return (
    <header className="studentHeader">
      <a
        href="/"
        className={classNames("studentHeader__burger laptop-hide", {
      ...
      <div className="header__actions">
        ...
        <div
          className="header__actions-item header__logout"
      ...
      {showConfirmLogout ? (
        <Confirm
          message={translate("sure_to_logout")}
          confirmAction={onConfirmLogout}
          cancelAction={() => setShowConfirmLogout(false)}
        />
      ) : null}
    </header>
  );
  ...
  function onHideMenu(e) {
    if (
      !e.target.closest(".studentHeader__burger") &&
      !e.target.closest(".studentNav")
    ) {
