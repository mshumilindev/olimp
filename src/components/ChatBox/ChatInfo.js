...
export default function ChatInfo({ isStatic, chat }) {
  ...
  return (
    <div
      ...
      style={
        isStatic
          ? { position: "static", marginBottom: 30, marginTop: -15 }
          : null
      }
    >
      <div className="chatroom__title-inner">
        {connection ? (
          <TextTooltip
            ...
            children={
              <div
                className={classNames("chatroom__connection", {
              ...
              >
                {connection === "offline" ? (
                  <span className="offlineAnimation">
      ...
      {chatCount > 0 ? (
        <div className="chatroom__timer">{makeChatCount()}</div>
      ) : null}
    </div>
  );
  ...
  function makeMinutes() {
    return chatCount / timerMultiplier < 10
      ? "0" + Math.floor(chatCount / timerMultiplier)
      : Math.floor(chatCount / timerMultiplier);
  }

  function makeSeconds() {
    return ("" + (chatCount % timerMultiplier)).length === 1
      ? "0" + (chatCount % timerMultiplier)
      : chatCount % timerMultiplier;
  }
}
