    ...
    if (displayName) {
      ...
      if (newUsersPresent.find((newUserItem) => newUserItem.id === id)) {
        setUsersPresent(
          Object.assign(
            [],
            newUsersPresent.filter((newUserItem) => newUserItem.id !== id)
              .length
              ? newUsersPresent.filter((newUserItem) => newUserItem.id !== id)
              : [],
          ),
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    setChatStart: (chatID, setStarted) =>
      dispatch(setChatStart(chatID, setStarted)),
    setStopChat: (chatID) => dispatch(setStopChat(chatID)),
    discardChat: () => dispatch(discardChat()),
    setOnACall: (value) => dispatch(setOnACall(value)),
    toggleChalkBoard: (chatID, value) =>
      dispatch(toggleChalkBoard(chatID, value)),
    toggleLesson: (chatID, value) => dispatch(toggleLesson(chatID, value)),
  };
};

export default compose(
  connect(mapStateToProps, mapDispatchToProps)(memo(ChatWidget)),
);
