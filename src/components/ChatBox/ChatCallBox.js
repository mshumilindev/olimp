...
const ChatCallBox = ({isOrganizer, events, chat, caller, setOnACall}) => {
  if ( isOrganizer || events.participant?.find(eventItem => eventItem.started)?.id !== chat?.id ) {
    return null;
  }

  return (
      <div className="quickCall">
          <div className="quickCall__info" style={{backgroundImage: `url(${quickCallBg})`}}>
              {
                  caller ?
                      <>
                        ...
                        {
                          caller.avatar ? (
                            <span className="quickCall__avatar" style={{backgroundImage: `url(${caller.avatar})`}} />
                          ) : null
                        }
