...
class Jitsi {
  ...
  start({
    ...
    onShareScreen = (tracks) => {
      ...
      if (room) {
        if (localTracks.find((item) => item.type === "video")) {
          ...
          room.replaceTrack(
            localTracks.find((item) => item.type === "video"),
            shareScreenTrack,
          );
      ...
      for (let i = 0; i < self.localTracks.length; i++) {
        if (!noUserCreation) {
          ...
          if (self.localTracks[i].getType() === "video") {
            $(containers.remote).append(
              `<div class="video localVideo"><video autoplay='1' muted playsinline id='localVideo' /></div>`,
            );
            self.localTracks[i].attach(
              containers.remote.querySelector(`#localVideo`),
            );
          }
        } else {
    ...
    function onRemoteTrack(track) {
      ...
      track.addEventListener(
        ...
        (track) => {
          const videoEl = document.querySelector(
            "." + track.containers[0].id.replace("audio", "video"),
          );
          const shareScreenEl = document.querySelector(
            "." + track.containers[0].id.replace("audio", "shareScreen"),
          );
      ...
      setTimeout(() => {
        ...
        if (track.videoType === "desktop") {
          ...
          $(containers.shareScreen).append(
            `<div class='video shareScreen${participant}'><video autoplay='1' muted playsinline id='shareScreen${participant}' /></div>`,
          );
          if (
            self.remoteTracks[track.ownerEndpointId] &&
            self.remoteTracks[track.ownerEndpointId].find(
              (item) => item.type === "audio",
            )
          ) {
            const videoEl = containers.remote.querySelector(
              ".shareScreen" + track.ownerEndpointId,
            );
            const isMuted = self.remoteTracks[track.ownerEndpointId].find(
              (item) => item.type === "audio",
            ).muted;
          ...
          if (track.getType() === "video") {
            if (!document.querySelector(".video" + participant)) {
              $(containers.remote).append(
                `<div class='video video${participant}'><video autoplay='1' muted playsinline id='video${participant}' /></div>`,
              );
              if (
                self.remoteTracks[track.ownerEndpointId] &&
                self.remoteTracks[track.ownerEndpointId].find(
            ...
            if (!document.getElementById("audio" + participant)) {
              $(containers.remote).append(
                `<audio autoplay='1' id='audio${participant}' />`,
              );
    ...
    function onConnectionSuccess() {
      room = self.connection.initJitsiConference(
        ("olimp_remote_school" + roomName).toLowerCase(),
        chatConfig.confOptions,
      );
      ...
      room.on(JitsiMeetJS.events.conference.MESSAGE_RECEIVED, (id, text) => {
        ...
        if (newText.event) {
          ...
          if ($usersContainer) {
            const $userConnected = $usersContainer.querySelector(
              '[data-user-name="' + newText.name + '"]',
            );
    ...
    connection = self.connection = new JitsiMeetJS.JitsiConnection(
      null,
      null,
      chatConfig.options,
    );
  ...
  startDevices(onlyVideo) {
    const selectedCameraID = localStorage.getItem("videoDevice")
      ? JSON.parse(localStorage.getItem("videoDevice")).id
      : null;
    ...
    function goThroughVideoDevices(devices, length) {
      if (devices[i].kind === "videoinput") {
        JitsiMeetJS.createLocalTracks({
          devices: ["audio", "video"],
          cameraDeviceId: devices[i].deviceId,
          resolution: 480,
        })
          .then((tracks) => onLocalTracks(tracks, onlyVideo))
          .catch((error) => {
  ...
  changeVideoDevice(value, label) {
    localStorage.setItem(
      "videoDevice",
      JSON.stringify({ id: value, label: label }),
    );
    JitsiMeetJS.createLocalTracks({
      devices: ["video"],
      ...
      if (localTracks.find((item) => item.type === "video")) {
        ...
        localTracks[
          localTracks.indexOf(localTracks.find((item) => item.type === "video"))
        ] = tracks[0];
      } else {
