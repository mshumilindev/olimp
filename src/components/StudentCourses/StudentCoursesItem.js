...
function StudentCoursesItem({ user, subjectID, courseID, currentUser, tests }) {
  ...
  return tests ? <div className="studentCourse">{_renderModules()}</div> : null;

  function _renderModules() {
    return (
      <div
        className="studentCourse__modules"
  ...
  function fetchModulesLessons() {
    ...
    getDocs(modulesRef).then((snapshot) => {
  ...
  function checkIfHasScore(moduleID, lessonID) {
    ...
    const foundTest = tests.find(
      (item) =>
        item.lesson.subjectID === subjectID &&
        item.lesson.courseID === courseID &&
