      ...
      {user ? (
        <>
          <div className="profile__avatar-holder">
            ...
            >
              {!user.avatar ? <i className="fas fa-camera-retro" /> : null}
            </div>
          </div>
          <div className="profile__info">
                ...
                <div className="profile__class">
                  {!!user?.class && !!classesList?.length ? (
                    classesList?.find((item) => item.id === user.class)?.title[
                      ...
                      classesList?.find((item) => item.id === user.class)
                        ?.title[lang]
                    ) : (
                      classesList?.find((item) => item.id === user.class)
                        ?.title["ua"]
                    ...
                    <div className="nothingFound">{translate("no_class")}</div>
                  )}
                </div>
              ) : null}
            </div>
            {user.role === "teacher" ? (
              <div className="profile__courses">
                <div className="profile__heading">{translate("courses")}</div>
                {checkForCourses().length ? (
                  checkForCourses().map((course) => {
                    return (
                      <div
                        className="profile__courses-item"
                        key={course.course.id}
                      >
                        <i className="fa fa-graduation-cap" />
                        {course.course.name[lang]
                          ? course.course.name[lang]
                          : course.course.name["ua"]}
                      </div>
                    );
                  })
                ) : (
            ...
            {user.email || user.tel || user.skype ? (
              <>
                <div className="profile__heading">{translate("contact")}</div>
