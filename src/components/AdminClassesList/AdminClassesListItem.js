...
function AdminClassesListItem({item, index, removeClass, usersList, user}) {
    ...
    const classTitle = useMemo(() =>  {
      ...
      if ( hasNumber.test(arr[0]) && arr.length > 2 && arr.some((item) => hasQuote.test(item)) ) {
      ...
      if ( arr.length > 2 ) {
        arr = [
          arr[0],
          arr[1],
          arr.splice(2, arr.length - 1).join(' ').trim()
        ];
      }
      ...
      if ( hasNumber.test(arr[0]) && arr.length > 1 ) {
        arr = [
          arr[0],
          arr.splice(1, arr.length - 1).join(' ').trim()
        ]
      }

      // Joining all items in case there are no numbers
      if ( !hasNumber.test(arr[0]) && !hasNumber.test(arr[1]) && !hasQuote.test(arr) ) {
        arr = [arr.join(' ')];
      }
    ...
    return (
      <AdminClassesListItemStyled>
        <AdminClassesListItemLinkStyled to={'/admin-classes/' + item.id}>
          <AdminClassesListItemLinkBGStyled style={{backgroundImage: `url(${classBg})`, backgroundPosition: `${getBgPosition()}% ${getBgPosition()}%`}} />
          {
            !!curator?.avatar && (<AdminClassesListItemCurator style={{backgroundImage: `url(${curator.avatar})`}} to={curator.id === user?.id ? `/admin-profile` : `/admin-users/${curator.login}`} />)
          }
          <AdminClassesListItemTitleStyled>
            {
              classTitle.map((item, index) => (
                <>
                  <AdminClassesListItemTitleLineStyled>{item}</AdminClassesListItemTitleLineStyled>
                  {
                    index === 1 && (
          ...
          <span className="adminClasses__list-item-remove" onClick={handleRemoveClass}>
            <TextTooltip text={translate('Видалити')} position="left" children={
              <i className="fa fa-trash-alt"/>
            } />
          </span>
        </AdminClassesListItemLinkStyled>
        {
          showConfirmRemove && <Confirm message={translate('sure_to_remove_class')} confirmAction={onConfirmRemove} cancelAction={() => setShowConfirmRemove(false)}/>
        }
      </AdminClassesListItemStyled>
    );
}
...
export default connect(mapStateToProps, mapDispatchToProps)(AdminClassesListItem);
