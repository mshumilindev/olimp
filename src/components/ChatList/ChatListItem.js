...
const ChatListItem = ({
  ...
  return (
    <div
      className={classNames("adminChats__event", { isActive: event.started })}
      key={event.id}
    >
      <span onClick={() => navigate("/chat/" + event.id)}>
        <div className="adminChats__event-time">
          {moment(event.datetime * 1000).format("HH:mm")}
          {event.started ? (
            <span className="adminChats__event-status">
              {translate("chat_active")}
            </span>
          ) : null}
        </div>
        <div className="adminChats__event-info">
          ...
          <div className="adminChats__event-info-row">
            <div className="adminChats__event-info-dt">
              {translate("organizer")}:{" "}
            </div>
            <div className="adminChats__event-info-dd">
              {getUser(event.organizer) && (
                <Link
                  to={
                    (isStudent ? "/user/" : "/admin-users/") +
                    getUser(event.organizer).login
                  }
                  onClick={(e) => e.stopPropagation()}
                >
                  {getUser(event.organizer).name}
                </Link>
              )}
            </div>
          </div>
            ...
            <div className="adminChats__event-info-dd">
              {typeof event.participants === "object" ? (
                  ...
                  if (getUser(partItem)) {
                    return (
                      <Link
                        to={
                          (isStudent ? "/user/" : "/admin-users/") +
                          getUser(partItem).login
                        }
                        key={partItem}
                        onClick={(e) => e.stopPropagation()}
                      >
                        {getUser(partItem).name}
                      </Link>
                    );
                  } else {
                    return null;
                ...
                <Link
                  to={
                    (isStudent ? "/user/" : "/admin-users/") +
                    getUser(event.participants).login
                  }
                  key={event.participants}
                  onClick={(e) => e.stopPropagation()}
                >
                  {getUser(event.participants).name}
                </Link>
              ) : null}
            </div>
          </div>
      ...
      {(!noActions && user.id === event.organizer) || user.role === "admin" ? (
        <div className="adminChats__event-actions">
          <span
            className="btn btn_primary round btn__xs"
            onClick={() => mapEventToFormFields(event)}
          >
      ...
      {showConfirmDelete ? (
        <Confirm
          ...
          message={translate("sure_to_delete_videochat")}
          confirmAction={() => deleteEvent(event.id)}
        />
      ) : null}
    </div>
  );
