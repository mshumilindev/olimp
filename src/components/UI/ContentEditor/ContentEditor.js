...
export default function ContentEditor({
  ...
  if (types.length) {
    types.forEach((defaultType) => {
      if (typeof defaultType === "object") {
        contentEditorActions.push(
          defaultType.map((innerType) => {
            if (defaultEditorActions.find((item) => item.type === innerType)) {
              return defaultEditorActions.find(
                (item) => item.type === innerType,
              );
            }
          }),
        );
        ...
        if (defaultEditorActions.find((item) => item.type === defaultType)) {
          contentEditorActions.push(
            defaultEditorActions.find((item) => item.type === defaultType),
          );
        }
      }
    });
    ...
    <div className="contentEditor">
      ...
      {showAddModal ? (
        <Modal onHideModal={() => setShowAddModal(false)}>
          <h2 className="contentEditor__actions-heading">
            {translate("choose_block_type")}
          </h2>
          <div className="contentEditor__actions">
            {contentEditorActions.length
              ? contentEditorActions.map((action, index) =>
  ...
  function _renderAction(action, index) {
    if (Array.isArray(action)) {
      return (
        <div
          className="contentEditor__actions-row"
          key={"" + index + action.type}
        >
          {action.map((innerAction, innerIndex) =>
            _renderAction(innerAction, innerIndex),
          )}
        </div>
      );
    } else {
      return (
        <a
          href="/"
          className="contentEditor__actions-link"
          onClick={(e) => startAddContentBlock(e, action.type)}
          key={"" + index + action.type}
        >
          <i className={action.icon} />
          {action.title ? translate(action.title) : null}
        </a>
      );
    }
  }
  ...
  function _renderNoContent() {
    return (
      <div className="contentEditor__noContent">
