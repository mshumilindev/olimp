    ...
    return (
        <div className="lessonPicker">
            <div className="lessonPicker__info">
                {
                    selectedLesson && selectedLesson.subjectID ?
                        <>
                            ...
                            <span className="lessonPicker__remove" onClick={() => setLesson(null)}>
            ...
            <div className="userPicker__add">
                <span className="userPicker__add-btn" onClick={() => setShowModal(true)}>
                    <i className="fa fa-pencil-alt"/>
                    <span className="userPicker__placeholder">{ translate('pick_lesson') }</span>
                </span>
            </div>
            {
                showModal ?
                    <Modal heading={translate('pick_lesson')} onHideModal={() => setShowModal(false)}>
                            ...
                            >
                                {
                                    coursesList ?
                                        filterCoursesList().length ?
                                            orderBy(filterCoursesList(), v => v.name[lang] ? v.name[lang] : v.name['ua']).map(subjectItem => <LessonPickerSubject subject={subjectItem} key={subjectItem.id} setLesson={handleSetLesson} pickedLesson={pickedLesson} />)
                                            :
                                            <div className="nothingFound">
                                                { translate('nothing_found') }
                                            </div>
