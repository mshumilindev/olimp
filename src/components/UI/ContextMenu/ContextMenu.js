...
export default function ContextMenu({ children, links, dontShow }) {
  ...
  return (
    <span
      className={classNames("contextmenu", { active: showContextMenu })}
    ...
    return link.type !== "divider" ? (
      <div className="contextmenu__drop-item" key={link.id}>
        <a
          href="/"
          className={
            "contextmenu__drop-link" +
            (link.type ? " link-" + link.type : "") +
            (link.disabled ? " disabled" : "")
          }
          onClick={(e) => handleLink(e, link.action, link.disabled)}
        >
  ...
  function hideContextMenu(e) {
    if (
      ...
      e.target.closest(".contextmenu") !== $trigger.current
    ) {
      toggleContextMenu(false);
    }
  }
