...
function LibraryPicker({
  ...
  return (
    <div className="libraryPicker">
      {selectedList.length ? (
        ...
        <div className="nothingFound">{translate("no_textbook")}</div>
      )}
      <div className="libraryPicker__add">
        <span
          className="libraryPicker__add-btn"
          onClick={() => setShowLibraryListModal(true)}
        >
          {selectedList.length ? (
          ...
          {placeholder ? (
            <span className="libraryPicker__placeholder">{placeholder}</span>
          ) : null}
        </span>
      </div>
      {showLibraryListModal ? (
        <Modal
          onHideModal={() => setShowLibraryListModal(false)}
          heading={translate("new") + " " + translate("textbook")}
        >
              ...
              libraryList.filter((item) =>
                ...
                <>
                  {filterLibraryList()
                    ...
                    .map((item) => _renderLibraryItem(item))}
                  {filterLibraryList()
                    .filter((item) => !selectedList.includes(item.id))
                    .map((item) => _renderLibraryItem(item))}
                </>
          ...
          {libraryList.filter(
            (item) =>
              user.role === "admin" || item.teacher.indexOf(user.id) !== -1,
          ) ? (
            <div className="libraryPicker__list-btn">
              <a
                href="/"
                className="btn btn_primary"
                onClick={(e) => onAddBooks(e)}
              >
                {selectedList.length ? (
                  <>
  ...
  function _renderSelectedBook(bookID) {
    ...
    return (
      <div className={"libraryPicker__list-item selectedBookItem"} key={bookID}>
        ...
        <span
          className="libraryPicker__list-item-remove"
          onClick={() => quickRemoveBook(book.id)}
        >
  ...
  function _renderLibraryItem(book) {
    return (
      <div
        className={classNames("libraryPicker__list-item", {
          selected: selectedBooks.some((item) => item === book.id),
        })}
  ...
  function quickRemoveBook(bookID) {
    ...
    if (booksList.length > 1) {
      addBooks(
        ...
        Object.assign(
          [],
          booksList.filter((item) => item !== bookID),
        ),
      );
    } else {
