...
export default function SeamlessEditorPreview({
  content,
  scrollToBlock,
  moveBlock,
  removeBlock,
  ...
  return (
    <div className="seamlessEditor__preview-holder">
      <Scrollbars
        ...
        renderView={(props) => (
          <div {...props} className="seamlessEditor__preview" />
        )}
      ...
      <div
        className={classNames("seamlessEditor__preview-delete", {
          isOver: isToDelete,
        })}
        onDragEnter={() => setIsToDelete(true)}
      >
  ...
  function _renderPreviewBlock(item, index) {
    return (
      <div
        className="seamlessEditor__preview-item-holder"
        onDragEnter={() => setDragOverBlock(item.id)}
        key={item.id}
      >
        {dragBlock && index === 0 ? _renderDropArea(item.id, "before") : null}
        <div
          className="seamlessEditor__preview-item"
          onClick={() => scrollToBlock(item.id)}
          draggable
          onDragStart={() => setDragBlock(item.id)}
  ...
  function _renderDropArea(blockID, position) {
    return (
      <div
        className={classNames("seamlessEditor__preview-dropArea", {
          isOver:
  ...
  function handleDragEnd() {
    if (isToDelete) {
      ...
      if (dragBlock && dragOverBlock && dragOverBlockPosition) {
        const prevIndex = content.indexOf(
          content.find((item) => item.id === dragBlock),
        );
        const newIndex = content.indexOf(
