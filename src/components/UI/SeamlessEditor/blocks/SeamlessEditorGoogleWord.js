...
export default function SeamlessEditorGoogleWord({ block, setBlock }) {
    ...
    useEffect(() => {
        setTimeout(() => {
            const width = wordContainerRef.current.offsetWidth - parseInt(getComputedStyle(wordContainerRef.current).paddingLeft);
    ...
    return (
        <div className="seamlessEditor__editor-block-word" ref={wordContainerRef}>
            <form className="form">
            ...
            <br/>
            {
                block.value && size.width > 0 ?
                    <div className="seamlessEditor__editor-block-word-holder">
                        <iframe
