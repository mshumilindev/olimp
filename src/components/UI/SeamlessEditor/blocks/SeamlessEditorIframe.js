...
export default function SeamlessEditorIframe({ block, setBlock }) {
  ...
  useEffect(() => {
    setTimeout(() => {
      const width =
        powerpointContainerRef.current.offsetWidth -
        parseInt(getComputedStyle(powerpointContainerRef.current).paddingLeft);
  ...
  return (
    <div
      className="seamlessEditor__editor-block-iframe"
      ref={powerpointContainerRef}
    >
      <form className="form">
  ...
  function handleChange(value) {
    const url = value.startsWith("<iframe")
      ? value
          .split(" ")
          .find((item) => item.startsWith('src="'))
          .replace('src="', "")
          .replace('"', "")
      : value;
