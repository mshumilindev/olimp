...
function CoursesPicker({
  ...
  const quickRemoveCourse = useCallback(
    (course) => {
      const newSelectedList = selectedList;

      newSelectedList.splice(
        newSelectedList.indexOf(
          newSelectedList.find(
            (item) =>
              item.subject === course.subject && item.course === course.course,
          ),
        ),
        1,
      );
    ...
    (subjectID, courseID) => {
      ...
      if (
        newSelectedList.some(
          (item) => item.subject === subjectID && item.course === courseID,
        )
      ) {
        newSelectedList.splice(
          newSelectedList.indexOf(
  ...
  const onAddClasses = useCallback(
    (e) => {
      e.preventDefault();
  ...
  const _renderCourse = useCallback(
    (subjectID, course) => {
      const parsedSelectedList = selectedList;

      return (
        <div
          className={classNames("coursesPicker__list-coursesList-item", {
            selected: parsedSelectedList.some(
              (item) => item.subject === subjectID && item.course === course.id,
            ),
          })}
          key={course.id}
  ...
  const _renderSubject = useCallback(
    (subject) => {
      return (
          ...
          {subject.coursesList.length ? (
            <div className="coursesPicker__list-coursesList">
              {subject.coursesList.map((course) =>
                _renderCourse(subject.id, course),
              )}
            </div>
          ) : (
  ...
  const _renderSelectedCourse = useCallback(
    (course) => {
      const currentSubject = coursesList.find(
        (item) => item.id === course.subject,
      );
      if (!currentSubject) return [];
      const currentCourse = currentSubject.coursesList.find(
        (item) => item.id === course.course,
      );
        ...
        <div className="coursesPicker__selectedList-item" key={course.course}>
          ...
          {!noControls ? (
            <span
              ...
              onClick={() => quickRemoveCourse(course)}
            >
    ...
    <div className="coursesPicker">
      ...
      {coursesList && coursesList.length && !noControls ? (
        <div className="coursesPicker__add">
          <span
            className="coursesPicker__add-btn"
      ...
      {showSubjectsListModal ? (
        <Modal
          ...
          heading={translate("add") + " " + translate("course")}
        >
