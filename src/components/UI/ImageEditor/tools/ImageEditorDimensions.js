...
export default function ImageEditorDimensions({originalDimensions, dimensions, setSettingsItem}) {
    ...
    function _renderDimension(item) {
        return (
            <div className={classNames('imageEditor__toolbar-btn', {active: dimensions.width / dimensions.height === item.width / item.height})} onClick={() => setSettingsItem('dimensions', {width: item.width, height: item.height})}>
    ...
    function closeDrop(e) {
        if ( e.target !== $dimensions.current && !$dimensions.current.contains(e.target) ) {
            setShowDrop(false);
        }
    }
}