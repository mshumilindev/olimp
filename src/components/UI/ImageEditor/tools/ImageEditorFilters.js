...
export default function ImageEditorFilters({
  image,
  filters,
  setSettingsItem,
}) {
  ...
  function _renderInstaFilter(item) {
    return (
      <div
        className={classNames("imageEditor__filters-insta-item", {
          active: item === filters.item,
        })}
        key={item}
        onClick={() => setSettingsItem("filters", { ...filters, item })}
      >
        <div
          className={"imageEditor__filters-insta-img imageFilter-" + item}
          style={{ backgroundImage: "url(" + image + ")" }}
        />
        <div className="imageEditor__filters-insta-label">
          {item === "normal" ? translate("normal") : item}
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="imageEditor__filters-filter" key={item.id}>
        ...
        <Range
          ...
          activeValue={
            item.increment
              ? parseInt(parseFloat(filters[item.id]) * item.increment)
              : parseInt(filters[item.id])
          }
          type={"filters"}
          setRange={(type, value) =>
            setSettingsItem(type, {
              ...filters,
              [item.id]: item.increment
                ? value / item.increment + item.units
                : value + item.units,
            })
          }
          units={item.units}
