...
export default function CustomSelect({
  ...
  return (
    <div className="customSelect" ref={$currentSelect}>
      ...
      >
        <input
          className={classNames("form__field customSelect__value", {
            isUpdated: updated,
            required: required,
            hasErrors: hasErrors,
            isActive: isActive,
        ...
        <div className="customSelect__drop">
          ...
          >
            {hasReset && value ? (
              <div
                className="customSelect__drop-opt isReset"
                onClick={() => selectOpt(id, "")}
              >
                {translate("reset")}
              </div>
            ) : null}
  ...
  function _renderOpt(opt) {
    return (
      <div
        className="customSelect__drop-opt"
        key={opt.id}
        onClick={() => selectOpt(id, opt.id)}
        id={opt.id}
      >
        {opt.icon ? (
          <i
  ...
  function handleBodyClick(e) {
    if (
      e.target !== $currentSelect.current &&
      e.target.closest(".customSelect") !== $currentSelect.current
    ) {
      setIsActive(false);
    }
  }
