...
function AdminSettingsContact({contacts, addBlock, removeBlock, updateModel}) {
    ...
    if ( contacts !== initialContacts ) {
        ...
        JSON.parse(contacts).sort((a, b) => a.order - b.order).forEach(item => {
            const newField = {
                type: 'block',
                id: item.id,
    ...
    return (
        <Form fields={contactFields} setFieldValue={updateModel} />
    )
}
