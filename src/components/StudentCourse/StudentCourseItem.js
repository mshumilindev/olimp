...
function StudentCourseItem({
    ...
    <div className="studentCourse">
      ...
      {!currentCourse || !usersList ? (
        ...
        <div className="grid">
          ...
          <div className="grid_col col-12 tablet-col-6 block">
            {libraryList && libraryList.length ? _renderTextbook() : null}
          </div>
          <div className="grid_col col-12 block">{_renderModules()}</div>
        </div>
      )}
    </div>
  );
      ...
      <div className="studentCourse__modules">
        ...
        {!modulesLessons || modulesLessonsLoading ? (
          ...
          modulesLessons
            ...
            .map((module) => _renderModule(module))
        ) : (
        ...
        <div className="studentCourse__module-lessons">
          {module.lessons.length ? (
            module.lessons
              ...
              .filter((lesson) => !lesson.hideLesson)
  ...
  function _renderLesson(moduleID, lesson) {
    return (
      <div className="studentCourse__module-lessons-item" key={lesson.id}>
        <div
          className={classNames("studentCourse__module-lessons-icon", {
            hasScore: checkIfHasScore(moduleID, lesson.id),
          })}
        >
        ...
        {checkIfHasScore(moduleID, lesson.id) ? (
          <div className="studentCourse__score">
            {translate("mark")}:{" "}
            <span>{checkIfHasScore(moduleID, lesson.id)}</span>
          </div>
        ) : null}
      </div>
    );
      ...
      <>
        ...
        {getTeacher() ? (
          <div className="studentCourse__teacher">
            <div
              className="studentCourse__teacher-avatar"
              style={{ backgroundImage: "url(" + getTeacher().avatar + ")" }}
            >
              {!getTeacher().avatar ? <i className="fa fa-user" /> : null}
            </div>
  ...
  function filterLibrary() {
    ...
    if (typeof currentCourse.course.textbook === "object") {
      ...
      if (
        libraryList.find(
          (libItem) => libItem.id === currentCourse.course.textbook,
        )
      ) {
        newLibrary.push(
          libraryList.find(
  ...
  function checkIfHasScore(moduleID, lessonID) {
    ...
    const foundTest = tests.find(
      (item) =>
        item.lesson.moduleID === moduleID &&
        item.lesson.lessonID === lessonID &&
        item.userID === user.id,
    );
    ...
    currentTeacher =
      ...
      usersList.find(
        (teacher) =>
          ...
          teacher.id === currentCourse.course.teacher,
      );
