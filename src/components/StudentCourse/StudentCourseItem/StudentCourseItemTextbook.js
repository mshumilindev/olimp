...
export default function StudentCourseItemTextbook({ docRef, name }) {
    ...
    return (
        <>
                        ...
                        <div className="studentCourse__textbook-item-holder">
                            {
                                checkIfMobile() || textbook.type === 'else' ?
                                    <a href={textbook.url} target="_blank" className="btn btn_primary">
                                        <i className="content_title-icon fa fa-book" />
                                        { translate('open_textbook') }
                                    </a>
                                    :
                                    <span className="btn btn_primary" onClick={() => setShowModal(true)}>
                                <i className="content_title-icon fa fa-book" />
                                        { translate('open_textbook') }
                            </span>
                            }
            ...
            {
                showModal ?
                    <Modal className="textbookModal" onHideModal={() => setShowModal(false)} heading={translate('textbook')}>
                        <object data={textbook.url} type="application/pdf" width="100%" height="100%">
