...
function AdminUsersListModal({
  ...
  const [userFields, setUserFields] = useState(
    getUserFields(currentUser, user, classesList, allCoursesList, []),
  );
      ...
      {showModal ? (
        <Modal onHideModal={() => toggleModal()}>
          <Form
            ...
            loading={loading}
            heading={translate("editing_user")}
            fields={userFields}
  ...
  function setFieldValue(fieldID, value) {
    ...
    if (fieldID === "login") {
      if (
        usersList.some((item) => item.id !== user.id && item.login === value)
      ) {
              ...
              child.children.forEach((blockChild) => {
                if (blockChild.updated) {
                  Object.assign(updatedFields, {
                    [blockChild.id]: blockChild.value,
                  });
                }
              });
            } else {
