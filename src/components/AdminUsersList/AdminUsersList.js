...
const AdminUsersList = ({
    ...
    <section className="section">
      ...
      <div className="adminUsersList grid">
        <div className="grid_col col-12">
            ...
            <>
              <div className="adminUsersList__list">
                {list
                  ...
                  .map((userItem) => _renderUsers(userItem))}
                {list
                  .filter((userItem) => userItem.status === "suspended")
                  .map((userItem) => _renderUsers(userItem))}
              </div>
              {pager}
            </>
          ...
          {list && list.length && loading ? <Preloader /> : null}
        </div>
      </div>
      {showConfirmRemove ? (
        <Confirm
          message={translate("sure_to_delete_user")}
          confirmAction={onConfirmRemove}
          cancelAction={hideShowConfirm}
        />
      ) : null}
    </section>
  );
  ...
  function _renderUsers(userItem) {
    ...
    const selectedClass = classesList
      ? classesList.find((item) => item.id === userItem.class)
      : null;
    ...
    return (
      <div
        className={classNames("adminUsersList__user", {
          nonActive: userItem.status !== "active",
        })}
        key={userItem.id}
      >
        <div
          className="adminUsersList__user-inner"
          onClick={() =>
            navigate(
              userItem.id !== user.id
                ? "/admin-users/" + userItem.login
                : "/admin-profile",
            )
          }
        >
          <div className="adminUsersList__user-status">
            <div
              className={"status status__" + userItem.status}
              title={translate(userItem.status)}
            />
          </div>
          <div className="adminUsersList__user-avatarHolder">
            <div
              className="adminUsersList__user-avatar"
          ...
          <div className="adminUsersList__user-info">
            <div className="adminUsersList__user-info-personal">
              <div className="adminUsersList__user-name">{userItem.name}</div>
            ...
            {selectedCourses.length ? (
              <div className="adminUsersList__user-courses">
                {orderBy(selectedCourses, (v) => v.courseName).map((course) => {
                  return (
                    <div
                      className="adminUsersList__user-courses-item"
                      key={course.courseName}
                    >
                      <i className="fa fa-book" />
                      {user.role === "admin" ? (
                        <Link
                          to={"/admin-courses/" + course.link}
                          onClick={(e) => e.stopPropagation()}
                        >
                          {course.courseName}
                        </Link>
        ...
        {user.role === "admin" ? (
          <div className="adminUsersList__user-actions">
            ...
            {userItem.id !== user.id ? (
              <a
                href="/"
                className="btn btn__error round btn__xs"
                onClick={(e) => handleDeleteUser(e, userItem.id)}
              >
