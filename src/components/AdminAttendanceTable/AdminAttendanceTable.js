...
const AdminAttendanceTable = ({
  ...
  const isWeekend = useCallback(
    (day) =>
      moment(day * 1000).format("dd") === "сб" ||
      moment(day * 1000).format("dd") === "нд",
    [],
  );
    ...
    (dayItem) => {
      ...
      return (
        <th className="table__head-cell" key={dayItem}>
          <div
            className={classNames("adminAttendance__day", {
          ...
          >
            <strong>
              ...
              <br />
              <span>{moment(dayItem * 1000).format("DD.MM")}</span>
            </strong>
          </div>
        </th>
      );
  ...
  const filterUsers = useCallback(
    (classItem) => {
      return orderBy(usersList, (v) => v.name)
        .filter(
          (userItem) =>
            userItem.role === "student" && userItem.status === "active",
        )
  ...
  const checkForMark = useCallback((dayAttCourses, lessonItem) => {
    return dayAttCourses.find(
      (item) =>
        (typeof item === "string" && item === lessonItem.course) ||
        (typeof item === "object" &&
          item.course === lessonItem.course &&
          item.start === lessonItem.time.start),
    );
  }, []);

  const addAttendance = useCallback(
    (userID, dayItem, courseItem, startTime, mark) => {
      ...
      const attDayIndex = attendance.find(
        (attItem) => attItem.day === formattedDayItem,
      )
        ? attendance.indexOf(
            attendance.find((attItem) => attItem.day === formattedDayItem),
          )
        : attendance.length;
      const attDay = attendance.find(
        (attItem) => attItem.day === formattedDayItem,
      ) || { day: formattedDayItem, courses: [] };

      if (
        checkForMark(attDay.courses, {
          course: courseItem,
          time: { start: startTime },
        })
      ) {
        if (mark) {
          let newDay = attDay.courses.find(
            (item) =>
              (typeof item === "string" && item === courseItem) ||
              (typeof item === "object" &&
                item.course === courseItem &&
  ...
  const handleMarkChange = useCallback(
    (userID, dayItem, courseItem, startTime) => (e) => {
  ...
  const subjectsByTeacher = useMemo(() => {
    if (user && coursesList) {
      const subjects = coursesList.filter((subject) =>
        subject.coursesList.some((course) => course.teacher === user.id),
      );
      const newSubjects = [];

      subjects.forEach((subject) => {
        if (
          subject.coursesList.filter((course) => course.teacher === user.id)
            .length
        ) {
          newSubjects.push({
            ...subject,
            coursesList: subject.coursesList.filter(
              (course) => course.teacher === user.id,
            ),
          });
        }
      });
  ...
  const classByTeacherSubjects = useCallback(
    (courses) => {
      return courses.some((course) =>
        subjectsByTeacher.some((teacherSubject) =>
          teacherSubject.coursesList.some(
            (teacherCourse) => teacherCourse.id === course.course,
          ),
        ),
      );
    },
  ...
  const _renderMark = useCallback(
    (lessonItem, index, dayAtt, dayItem, userItem) => {
      const foundCourse = coursesList
        ?.find((subjectItem) => subjectItem.id === lessonItem.subject)
        ?.coursesList?.find(
          (courseItem) => courseItem.id === lessonItem.course,
        );

      if (
        user.role !== "admin" &&
        showOnlyMy &&
        foundCourse?.teacher !== user.id
      ) {
        return null;
      }
      ...
      return (
        <div
          className={classNames("adminAttendance__check", {
            isWeekend: isWeekend(dayItem),
            isChecked:
              dayAtt &&
              dayAtt.courses &&
              checkForMark(dayAtt.courses, lessonItem),
          })}
          key={userItem.id + "_" + dayItem + "_" + lessonItem.course + index}
        >
          <div className="adminAttendance__check-title">
            {foundCourse?.name[lang]
              ? foundCourse?.name[lang]
              : foundCourse?.name["ua"]}
          </div>
  ...
  */}
          {
            <div
              className={classNames("adminAttendance__check-inner")}
              onClick={() =>
                addAttendance(
                  userItem.id,
                  dayItem,
                  lessonItem.course,
                  lessonItem.time.start,
                )
              }
            >
          ...
          <div
            className={classNames("adminAttendance__mark-input form", {
              isFilled:
                dayAtt &&
                dayAtt.courses &&
                checkForMark(dayAtt.courses, lessonItem) &&
                checkForMark(dayAtt.courses, lessonItem).mark,
            })}
          >
            <input
              type="number"
  ...
  const _renderCells = useCallback(
    (dayItem, dayLessons, userItem) => {
      const userAtt = userItem.attendance;
      let dayAtt = null;

      if (userAtt) {
        dayAtt = userAtt.find(
          (item) => item.day === moment(dayItem * 1000).format("DD_MMMM_YYYY"),
        );
      }
      ...
      return (
        <td className="table__body-cell" key={dayItem}>
          {orderBy(dayLessons, (v) => v.time.start).map((lessonItem, index) =>
            _renderMark(lessonItem, index, dayAtt, dayItem, userItem),
          )}
        </td>
      );
    },
      ...
      return (
        <tr className="table__body-row" key={userItem.id} title={userItem.name}>
          <td className="table__body-cell">
            <div className="adminAttendance__user">{userItem.name}</div>
          </td>
          {days
            ? days.map((dayItem) =>
                _renderCells(
  ...
  const _renderClass = useCallback(
    (classItem) => {
      if (
        user.role !== "admin" &&
        showOnlyMy &&
        !classByTeacherSubjects(classItem.courses)
      ) {
        return null;
      }
      ...
      return (
        <Fragment key={classItem.id}>
          <tr
            className="table__body-row adminAttendance__classRow"
            title={
              classItem.title[lang]
                ? classItem.title[lang]
                : classItem.title["ua"]
            }
          >
          ...
          {filterUsers(classItem.id).map((userItem) =>
            _renderRow(userItem, classItem.schedule),
          )}
        </Fragment>
      );
    },
      ...
      <table className="table">
        ...
        <thead className="table__head">
          <tr className="table__head-row">
            ...
            {days ? days.map((dayItem) => _renderHead(dayItem)) : null}
          </tr>
        </thead>
