...
export default function Datepicker({
  ...
  return (
    <div className="datepicker form__holder">
      {label ? <div className="form__field-label">{label}</div> : null}
      <div className="form">
        {date ? (
            ...
            {time ? (
              <div className="form__field-holder">
                <CustomSelect
                  ...
                  selectChanged={(fieldID, value) => handleTimeChange(value)}
  ...
  function handleChange(value, maxLength, type) {
    ...
    if (type === "day") {
      if (
        parseInt(newValue) >
        daysInMonth(parseInt(date.month), parseInt(date.year))
      ) {
        newValue = "" + daysInMonth(parseInt(date.month), parseInt(date.year));
      }
      onChange(
        moment(
          date.month + "." + newValue + "." + date.year + " " + date.time,
        ).unix(),
      );
    }
    if (type === "month") {
      ...
      if (
        parseInt(date.day) >
        daysInMonth(parseInt(newValue), parseInt(date.year))
      ) {
        onChange(
          moment(
            newValue +
        ...
        onChange(
          moment(
            newValue + "." + date.day + "." + date.year + " " + date.time,
          ).unix(),
        );
      }
    }
    if (type === "year") {
      if (
        parseInt(date.day) >
        daysInMonth(parseInt(date.month), parseInt(newValue))
      ) {
        onChange(
          moment(
            date.month +
        ...
        onChange(
          moment(
            date.month + "." + date.day + "." + newValue + " " + date.time,
          ).unix(),
        );
      }
    }
