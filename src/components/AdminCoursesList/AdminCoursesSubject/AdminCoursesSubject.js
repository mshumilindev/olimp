import React, { useContext, useState, useEffect, memo, useMemo, useCallback } from 'react';
...
import {deleteSubject, fetchCoursesList, deleteCourse} from "../../../redux/actions/coursesActions";
...
function AdminCoursesSubject({user, loading, subject, params, deleteSubject, isLessonCoppied, setIsLessonCoppied, fetchCoursesList, subjectCoursesList, deleteCourse, subjectCoursesListLoading, subjectIDRef, coursesQty = 0}) {
    ...
    useEffect(() => {
      const isCurrentSubjectOpen = !!params?.subjectID && params.subjectID === subject?.id;

      if ( subjectIDRef?.current !== params?.subjectID && ((isCurrentSubjectOpen && !params?.courseID) || (isCurrentSubjectOpen && !subjectCoursesList)) ) {
    ...
    const sortedCoursesList = useMemo(() => {
      const arr = subjectCoursesList?.filter((item) => user.role === 'teacher' ? item.teacher === user?.id : true);
    ...
    return (
      <AdminCoursesSubjectStyled id={isSubjectOpen ? 'scrollHere' : null}>
        <ContextMenu links={contextLinks}>
          <SubjectLinkStyled to={subjectLink} isOpen={isSubjectOpen} isSubjectSelected={params?.subjectID}>
            <span>
              {
                  checkIfIsOpen() ?
                      loading ?
            ...
            <AdminCoursesListItemCoursesStyled isEmpty={!coursesQty}>Кількість предметів: {coursesQty}</AdminCoursesListItemCoursesStyled>
          </SubjectLinkStyled>
        </ContextMenu>
        {
          isSubjectOpen && (
        ...
        {
            showUpdateCourse ?
                <UpdateCourse params={params} subjectID={subject.id} course={showUpdateCourse === 'edit' ? selectedCourse : null} loading={loading} setShowUpdateCourse={setShowUpdateCourse}/>
                :
                null
        }
        {
            showConfirm ?
                <Confirm message={translate('sure_to_delete_subject')} cancelAction={() => setShowConfirm(false)} confirmAction={handleDeleteSubject} />
                :
                null
        }
        {
            showUpdateModule ?
                <UpdateModule params={params} subjectID={params.subjectID} courseID={params.courseID} module={null} loading={loading} setShowUpdateModule={setShowUpdateModule}/>
                :
                null
        }
...
export default compose(connect(mapStateToProps, mapDispatchToProps), withRouter)(memo(AdminCoursesSubject));
