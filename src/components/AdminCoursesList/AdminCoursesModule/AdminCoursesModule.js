...
const AdminCoursesModule = ({
  ...
  const processDeleteModule = useCallback(() => {
    ...
    navigate(`/admin-courses/${subjectID}/${courseID}`);
  ...
  useEffect(() => {
    if (editOrder) {
      setNewOrder(
        Object.assign(
          [],
          sortLessons().map((item) => item.index),
        ),
      );
    } else {
      if (newOrder) {
  ...
  return (
    <div
      className={classNames("adminCourses__list-item", {
        someOpen: params && params.moduleID && params.moduleID !== module.id,
        isOpen: params && params.moduleID === module.id,
      })}
      style={{ padding: "5px 0" }}
    >
      ...
      {showUpdateLesson ? (
        <UpdateLesson
          ...
          lesson={null}
          loading={loading}
          setShowUpdateLesson={setShowUpdateLesson}
        />
      ) : null}
      {showConfirm ? (
        <Confirm
          message={translate("sure_to_delete_module")}
          cancelAction={() => setShowConfirm(false)}
          confirmAction={processDeleteModule}
        />
      ) : null}
    </div>
  );
    ...
    return (
      <div className="adminCourses__list-courses" style={{ marginTop: -10 }}>
        ...
        {lessonsList && lessonsList.length ? (
          ...
          <div
            className="adminCourses__list-item adminCourses__list-item-nothingFound"
  ...
  function sortLessons() {
    if (newOrder && editOrder) {
      return newOrder.map((newItem) =>
        lessonsList.find((item) => item.index === newItem),
      );
    } else {
...
const mapDispatchToProps = (dispatch) => ({
  fetchLessons: (subjectID, courseID, moduleID) =>
    dispatch(fetchLessons(subjectID, courseID, moduleID)),
  deleteModule: (subjectID, courseID, moduleID) =>
    dispatch(deleteModule(subjectID, courseID, moduleID)),
  updateLessonsOrder: (subjectID, courseID, moduleID, orderedLessons) =>
    dispatch(updateLessonsOrder(subjectID, courseID, moduleID, orderedLessons)),
