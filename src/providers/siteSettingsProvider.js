...
export default class SiteSettingsProvider extends React.Component {
  constructor() {
    super();
    this.state = {
      siteName: localStorage.getItem("siteSettings")
        ? JSON.parse(localStorage.getItem("siteSettings")).data.data.siteName[
      ...
      translate: (term) => {
        return !this.state.translations[this.state.lang] ||
          !this.state.translations[this.state.lang][term]
          ? term
          : this.state.translations[this.state.lang][term];
      },
      identify: (value) => {
  ...
  componentDidUpdate(prevProps, prevState, snapshot) {
    if (
      ...
      !this.state.translations[this.state.lang]
    ) {
      this.getTranslations();
    }
  }
  ...
  getUpdates() {
    ...
    getDocs(updatesCollection).then((snapshot) => {
      const translationsUpdates = snapshot.docs.find(
        (doc) => doc.id === "translations",
      );

      if (savedUpdates) {
        if (
          translationsUpdates.exists &&
          (!savedUpdates.translations ||
            translationsUpdates.data().date > savedUpdates.translations)
        ) {
  ...
  getTranslations() {
    ...
    if (!localStorage.getItem("translations" + lang)) {
      ...
      newTranslations[lang] = JSON.parse(
        localStorage.getItem("translations" + lang),
      );
      this.setState(() => {
        return {
          translations: newTranslations,
