...
const newSchedule = [];
const scheduleCollection = collection(db, "levels/" + level + "/schedule");

export function fetchSchedule() {
  if (!newSchedule.length) {
    return (dispatch) => {
      dispatch(fetchScheduleBegin());
      return getDocs(scheduleCollection).then((data) => {
...
const levelCoursesCollection = collection(db, "levels/" + level + "/courses");

export function fetchLevelCourses() {
  if (!newLevelCourses.length) {
    return (dispatch) => {
      dispatch(fetchLevelCoursesBegin());
      return getDocs(levelCoursesCollection).then((data) => {
        data.docs.map((doc) =>
          newLevelCourses.push({ [doc.id]: doc.data().course }),
        );
...
export function fetchCalendar() {
  if (!Object.keys(newCalendar).length) {
    return (dispatch) => {
      ...
      return getDocs(calendarCollection).then((data) => {
