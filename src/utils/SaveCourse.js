...
const withSaveCourse = (WrappedComponent) => {
  return class SaveCourse extends React.Component {
    ...
    render() {
      return (
        <WrappedComponent
          getCourseToSave={this.getCourseToSave}
          {...this.state}
          {...this.props}
        />
      );
    ...
    set coursesToSave(query) {
      if (
        !this.coursesToSaveArr.some((item) => item.subject === query.subject)
      ) {
    ...
    saveCourses() {
      ...
      const coursesListFromStorage = localStorage.getItem("courses")
        ? JSON.parse(localStorage.getItem("courses")).data
        : [];

      if (
        this.coursesToSave.length &&
        !coursesList.length &&
        !coursesListFromStorage.length
      ) {
        this.coursesToSave.map((item) => {
          if (
            !coursesList.some(
              (savedCourse) => savedCourse.id === item.courseID,
            ) &&
            !coursesListFromStorage.some(
              (savedCourse) => savedCourse.id === item.courseID,
            )
          ) {
            const courseRef = doc(
              "courses/" + item.subject + "/coursesList/" + item.courseID,
            );
