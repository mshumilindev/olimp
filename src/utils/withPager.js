...
const withPager = WrappedComponent => {
    return function Pager(props) {
        ...
        useEffect(() => {
            if ( props.list ) {
                if ( JSON.stringify(prevList) !== JSON.stringify(props.list) || prevLimit !== props.showPerPage ) {
        ...
        return <WrappedComponent {...props} list={pagedList(props.list)} pager={getPager()} />;

        function pagedList() {
            let newList = props.list;

            if ( newList ) {
                newList = JSON.parse(JSON.stringify(newList)).splice(limit * offset, limit);
            }
                ...
                <div className="pager" hidden={totalPages <= 1}>
                    {
                        pagerArr.map(item => {
                            return (
                                <span className={classNames('pager__item', {active: item.id === offset + 1})} key={item.id} onClick={() => setOffset(item.id - 1)}>
                                    { item.id }
                                </span>
                            )
                        })
