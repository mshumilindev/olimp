...
const withTags = WrappedComponent => {
    return function Tags(props) {
        ...
        return <WrappedComponent {...props} setTags={(tag) => onSetTags(tag)} list={filterListByTags()} selectedTags={_renderSelectedTags()} />;
            ...
            return (
                tagsList.length ?
                    <div className="tagsList selectedTags">
                        <h2 className="selectedTags__heading">{ translate('find_by_tags') }:</h2>
                        {
                            tagsList.map(tag => <span className="tagsList__item selectedTags__item" key={tag} onClick={() => removeTag(tag)}>{ tag }</span>)
                        }
                    </div>
                    :
                    null
