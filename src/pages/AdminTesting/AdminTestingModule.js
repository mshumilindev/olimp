...
const AdminTestingModule = ({
  ...
  const lessonsWithTests = useMemo(() => {
    return orderBy(
      lessonsList?.filter(({ id }) =>
        tests?.find((test) => test.lesson.lessonID === id),
      ),
      (v) => v.index,
    );
  }, [lessonsList, tests]);
            ...
            <AdminTestingLesson
              ...
              tests={tests.filter(
                (item) =>
                  ...
                  item.lesson.moduleID === module.id &&
                  item.lesson.lessonID === lesson.id,
              )}
              key={lesson.id}
              testID={testID}
            />
...
const mapDispatchToProps = (dispatch) => ({
  getLessons: (subjectID, courseID, moduleID) =>
    dispatch(getLessons(subjectID, courseID, moduleID)),
});
