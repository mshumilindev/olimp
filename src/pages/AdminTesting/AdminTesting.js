...
function AdminTesting({user, coursesList, tests, testID, usersList, deleteTests, loading}) {
    const { translate, lang } = useContext(siteSettingsContext);
    const [ activeTab, setActiveTab ] = useState('toCheck')

    useEffect(() => {
      if ( tests && usersList ) {
        const oldTests = tests.filter((test) => !usersList.find(({id}) => test.userID === id)).map((test) => test.id);
    ...
    const filterCourses = useCallback(() => {
      return (
        orderBy(coursesList, v => v.name[lang] ? v.name[lang] : v.name['ua'])
          .filter((subjectItem) => subjectItem.coursesList.some(courseItem => courseItem.teacher === user.id))
          .filter((subjectItem) => tests.some(testItem => testItem.lesson.subjectID === subjectItem.id))
      );
    }, [coursesList, lang, tests, user]);

    const mySubjects = useMemo(() => {
      return (
        orderBy(coursesList, v=> v.name[lang] || v.name['ua'])
          .filter((subjectItem) => subjectItem.coursesList.some((courseItem) => courseItem.teacher === user.id))
          .filter((subjectItem) => tests?.some((test) => test.lesson.subjectID === subjectItem.id))
          .map((subjectItem) => {
            return {
              ...subjectItem,
              coursesList: subjectItem.coursesList.filter((courseItem) => courseItem.teacher === user.id)
            }
          })
      )
    ...
    const myTests = useMemo(() => {
      const arr = tests?.filter((test) => mySubjects?.find((subjectItem) => subjectItem.coursesList.some((courseItem) => courseItem.id === test.lesson.courseID)))?.filter((test) => usersList.some((userItem) => userItem.id === test.userID && userItem.status === 'active'))
    ...
    const counter = useMemo(() => {
      const arr = tests?.filter((test) => mySubjects?.find((subjectItem) => subjectItem.coursesList.some((courseItem) => courseItem.id === test.lesson.courseID)))?.filter((test) => usersList.some((userItem) => userItem.id === test.userID && userItem.status === 'active'))
      const toCheck = arr?.filter((test) => test.isSent && !test.score)?.length || 0;
      const checked = arr?.filter((test) => test.isSent && test.score)?.length || 0;
    ...
    const filterTests = useCallback((subjectID, courseID) => {
      return myTests?.filter((test) => test.lesson.subjectID === subjectID && test.lesson.courseID === courseID);
    }, [myTests]);
              ...
              <div className="widget">
                <AdminTestingTabsHolderStyled>
                  <AdminTestingTabsStyled>
                    <AdminTestingTabStyled onClick={() => setActiveTab('toCheck')} active={activeTab === 'toCheck'}>
                      На перевірку ({counter.toCheck})
                    </AdminTestingTabStyled>
                    <AdminTestingTabStyled onClick={() => setActiveTab('checked')} active={activeTab === 'checked'}>
                      Перевірені ({counter.checked})
                    </AdminTestingTabStyled>
                    <AdminTestingTabStyled onClick={() => setActiveTab('saved')} active={activeTab === 'saved'}>
                            ...
                            <AdminTestingCoursesListStyled>
                              {
                                subject.coursesList.map((course) => {
                                  if ( !!filterTests(subject.id, course.id)?.length ) {
                                    return <AdminTestingCourse subjectID={subject.id} key={course.id} course={course} tests={filterTests(subject.id, course.id)} testID={testID} />;
                                  }
                                })
                              }
                            </AdminTestingCoursesListStyled>
