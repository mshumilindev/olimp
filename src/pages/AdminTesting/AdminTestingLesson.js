        ...
        {lesson.name[lang] || lesson.name['ua']}
        {
          !!sortedTests?.length && (
            <AdminTestingTestsListStyled>
              { sortedTests.map((test) => <AdminTestingTest test={test} key={test.id} userItem={usersList.filter(userItem => userItem.role === 'student' && userItem.id === test.userID)[0]} lesson={lesson} testID={testID} />) }
            </AdminTestingTestsListStyled>
          )
        }
      </AdminTestingLessonStyled>
