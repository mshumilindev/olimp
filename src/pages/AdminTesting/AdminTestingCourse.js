...
const AdminTestingCourse = ({subjectID, course, tests, testID, getModules}) => {
    ...
    useEffect(() => {
      if ( !modulesList?.length ) {
        getModules(subjectID, course.id).then((snapshot) => {
          setModulesList(snapshot.docs.map((doc) => {
            return {
              ...doc.data(),
              id: doc.id
            }
    ...
    const modulesWithTests = useMemo(() => {
      return orderBy(modulesList?.filter(({id}) => tests?.find((test) => test.lesson.moduleID === id)), v => v.index)
    }, [modulesList, tests]);
        ...
        </AdminTestingCourseStyled>
        {
          !!modulesWithTests?.length && (
            <AdminTestingModulesListStyled>
              { modulesWithTests.map((module) => <AdminTestingModule subjectID={subjectID} courseID={course.id} module={module} tests={tests} key={module.id} testID={testID}/>) }
            </AdminTestingModulesListStyled>
          )
        }
      </>
...
const mapDispatchToProps = dispatch => ({
  getModules: (subjectID, courseID) => dispatch(getModules(subjectID, courseID))
});
