...
function AdminUsers({
  ...
  const filterUsersList = useCallback(() => {
    return usersList && filterBy.length
      ? usersList
          .filter((user) => {
            const returnValues = [];
            const filterByRole = filterBy.find(
              (filter) => filter.id === "filterByRole",
            );
            const filterByStatus = filterBy.find(
              (filter) => filter.id === "filterByStatus",
            );
          ...
          .sort((a, b) => {
            const aValue =
              a[sortBy.value] === "admin"
                ? "a"
                : a[sortBy.value] === "teacher"
                  ? "b"
                  : a[sortBy.value] === "student"
                    ? "c"
                    : a[sortBy.value];
            const bValue =
              b[sortBy.value] === "admin"
                ? "a"
                : b[sortBy.value] === "teacher"
                  ? "b"
                  : b[sortBy.value] === "student"
                    ? "c"
                    : b[sortBy.value];
...
export default connect(mapStateToProps)(
  withFilters(AdminUsers, true, true, sortByOptions, filterByOptions),
);
