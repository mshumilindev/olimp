...
import {
  ...
  removePage,
  createPage,
} from "../../redux/actions/staticInfoActions";
...
function AdminPages({
  ...
  const { translate, lang, transliterize, identify } =
  ...
  const hideModal = useCallback(() => {
    ...
    initialNewPage,
    setNewPageFields,
    initialNewPageFields,
    setShowCreatePage,
  ]);

  const handleNewPageValue = useCallback(
    (fieldID, value) => {
      const tempNewPage = newPage;

      newPageFields.find((field) => field.id === fieldID).value = value;
      newPageFields.find((field) => field.id === fieldID).updated =
        !!value.length;
  ...
  const startCreatePage = useCallback(
    (e) => {
      e.preventDefault();
  ...
  const filterPages = useCallback(() => {
    ...
    if (list) {
      if (editedSearchQuery.trim()) {
        newPages = list.filter(
          (item) =>
            item.name.ua.toLowerCase().includes(editedSearchQuery) ||
            item.name.ru.toLowerCase().includes(editedSearchQuery) ||
            item.name.ua.toLowerCase().includes(editedSearchQuery),
        );
      }
    }
                  ...
                  <div className="grid_col col-3 desktop-col-2" key={item.id}>
                    <div className="adminPages__list-item">
                      <Link
                        ...
                        className="adminPages__list-link"
                      >
                        <div
                          className="adminPages__list-featured"
                          style={{
                      ...
                      <span
                        className="adminPages__list-remove"
                        onClick={() => setShowRemovePage(item.id)}
                      >
                    ...
                    {showRemovePage === item.id ? (
                      <Confirm
                        message={translate("sure_to_remove_page")}
                        confirmAction={() => handleRemovePage(item.id)}
                        cancelAction={() => setShowRemovePage(false)}
                      />
                    ) : null}
                  </div>
                );
      ...
      {showCreatePage ? (
        <Modal onHideModal={hideModal}>
          <Form
            ...
            heading={translate("create_page")}
            setFieldValue={handleNewPageValue}
            formAction={handleCreatePage}
