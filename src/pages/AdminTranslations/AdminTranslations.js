...
const AdminTranslationsList = React.lazy(
  () => import("../../components/AdminTranslationsList/AdminTranslationsList"),
);
  ...
  const filterTranslations = useCallback(() => {
    ...
    if (translationsList) {
      if (editedSearchQuery.trim()) {
        newTranslations = translationsList.filter(
          (item) =>
            item.id.toLowerCase().includes(editedSearchQuery) ||
            item.langs.some(
              (lang) =>
                lang[Object.keys(lang)] &&
                lang[Object.keys(lang)]
                  .toLowerCase()
                  .includes(editedSearchQuery),
            ),
        );
      }
    }
      ...
      if (updatedFields.length) {
        if ([...updatedFields].every((field) => field.value)) {
          updatedFields.forEach((field) => {
            ...
            const key = field.title.substr(
              field.title.indexOf("_") + 1,
              field.length,
            );
          ...
          <div className="section__title-actions">
            <span>
              <a
                ...
                onClick={(e) => handleUpdateTranslations(e)}
              >
...
const mapDispatchToProps = (dispatch) => ({
  updateTranslation: (lang, key, value) =>
    dispatch(updateTranslation(lang, key, value)),
  setUpdates: (type) => dispatch(setUpdates(type)),
});
export default connect(
  mapStateToProps,
  mapDispatchToProps,
)(withFilters(AdminTranslations, true, true));
