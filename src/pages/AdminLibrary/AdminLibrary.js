...
function AdminLibrary({
  ...
  const uploadFields = getDocFormFields(
    newFile.name,
    newFile.tags,
    newFile.teacher,
    translate("upload"),
            ...
            showOnlyMy
              ? item.teacher &&
                (item.teacher === user.id ||
                  item.teacher.indexOf(user.id) !== -1)
              : true,
          )
          ?.filter((item) =>
    ...
    <div className="adminLibrary">
      ...
      {showModal ? (
        <>
          <Modal onHideModal={onHideModal}>
            <Form
              heading={translate("upload_doc")}
              fields={uploadFields}
              formAction={handleUploadFile}
      ...
      <div className="section">
        <div className="section__title-holder">
          ...
          {!libraryList ? <Preloader size={60} /> : null}
        </div>
        {filters}
        <AdminLibraryList
          list={filterList()}
      ...
      {showConfirmRemove ? (
        <Confirm
          message={translate("sure_to_delete_doc")}
