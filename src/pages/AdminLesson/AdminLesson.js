...
function AdminLesson({
  ...
  useEffect(() => {
    setCurrentContent(
      JSON.parse(JSON.stringify(orderBy(lessonContent, (v) => v.index))),
    );
  }, [lessonContent]);
  ...
  useEffect(() => {
    if (currentContent) {
      if (
        ...
        JSON.stringify(orderBy(lessonContent, (v) => v.index))
      ) {
  ...
  useEffect(() => {
    if (currentQA) {
      if (
        JSON.stringify(currentQA) !==
        JSON.stringify(orderBy(lessonQA, (v) => v.index))
      ) {
        setQAUpdated(true);
      } else {
        setQAUpdated(false);
  ...
  useEffect(() => {
    window.onbeforeunload =
      (lessonUpdated || contentUpdated || QAUpdated) &&
      (() => translate("save_before_leaving"));
  });
  ...
  const getBreadcrumbs = useCallback(() => {
    ...
    if (params) {
      ...
      if (params.subjectID) {
        currentSubject = allCoursesList.find(
          (item) => item.id === params.subjectID,
        );
        if (
          !breadcrumbs.some(
            (item) => item.url === "/admin-courses/" + params.subjectID,
          )
        ) {
          breadcrumbs.push({
            name: currentSubject
      ...
      if (params.courseID) {
        if (currentSubject && currentSubject.coursesList) {
          currentCourse = currentSubject.coursesList.find(
      ...
      if (params.moduleID) {
        if (currentCourse && currentCourse.modules) {
          currentModule = currentCourse.modules.find(
          ...
          if (
            ...
            breadcrumbs.push({
              name: currentModule
                ? currentModule.name[lang]
                  ? currentModule.name[lang]
                  : currentModule.name["ua"]
                : "",
              url:
                "/admin-courses/" +
                params.subjectID +
    ...
    (fieldID, value) => {
      ...
      if (fieldID === "lessonName_ua") {
        if (lessonMeta.name.ua !== value) {
          newLessonInfoFields.find((field) => field.id === fieldID).updated =
            true;
    ...
    (e) => {
      ...
      if (lessonUpdated) {
        ...
        const newMeta = {
          ...lessonMeta,
          name: {
            ua: updatedLessonFields.find(
              (field) => field.id === "lessonName_ua",
            ).value,
            ru: updatedLessonFields.find(
              (field) => field.id === "lessonName_ru",
            ).value,
            en: updatedLessonFields.find(
              (field) => field.id === "lessonName_en",
            ).value,
          },
        };
  ...
  return lessonMeta && allCoursesList ? (
    <div className="adminLesson section">
        ...
        {checkIfEditable ? (
          <div className="section__title-actions">
            <a
              ...
              disabled={!lessonUpdated && !contentUpdated && !QAUpdated}
              onClick={saveLesson}
            >
      ...
      <div className="grid">
        <div className="grid_col col-8" style={{ maxWidth: 880 }}>
          <AdminLessonContent
            ...
            title={
              lessonMeta.name[lang]
                ? lessonMeta.name[lang]
                : lessonMeta.name["ua"]
            }
            setUpdated={setContentUpdated}
      ...
      {showPreview ? (
        <Modal
          className="adminLesson__previewModal"
          heading={
            lessonMeta.name[lang]
              ? lessonMeta.name[lang]
              : lessonMeta.name["ua"]
          }
          onHideModal={() => setShowPreview(false)}
        >
      ...
      {/* <Prompt
        when={lessonUpdated || contentUpdated || QAUpdated}
        message={translate("save_before_leaving")}
      /> */}
    </div>
  ) : (
    <Preloader />
...
const mapDispatchToProps = (dispatch) => ({
  fetchLessonMeta: (subjectID, courseID, moduleID, lessonID) =>
    dispatch(fetchLessonMeta(subjectID, courseID, moduleID, lessonID)),
  fetchLessonContent: (subjectID, courseID, moduleID, lessonID) =>
    dispatch(fetchLessonContent(subjectID, courseID, moduleID, lessonID)),
  fetchLessonQA: (subjectID, courseID, moduleID, lessonID) =>
    dispatch(fetchLessonQA(subjectID, courseID, moduleID, lessonID)),
  updateLessonMeta: (subjectID, courseID, moduleID, lessonID, newMeta) =>
    dispatch(
      updateLessonMeta(subjectID, courseID, moduleID, lessonID, newMeta),
    ),
  updateLessonContent: (subjectID, courseID, moduleID, lessonID, newContent) =>
    dispatch(
      updateLessonContent(subjectID, courseID, moduleID, lessonID, newContent),
    ),
  updateLessonQA: (subjectID, courseID, moduleID, lessonID, newQA) =>
