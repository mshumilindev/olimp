...
function Login({ loading, error, loginUser }) {
  ...
  const removeSavedUser = useCallback(
    (user) => (e) => {
      e.stopPropagation();
      const newSavedUsers =
        savedUsers.filter((savedUser) => savedUser.login !== user.login) || [];
        ...
        <div className="login__box">
          {window.location.pathname === "/suspended" ? (
            <div className="loginError">
              {translate("account_suspended_or_removed")}
            </div>
          ) : null}
          <LanguageSelect />
          ...
          {!!savedUsers?.length && (
            <div className="login__saved">
              {savedUsers.map((user) => (
                <div
                  className="login__saved-user"
                  onClick={() => loginUser(user.login, user.password)}
                >
                  <div
                    className="login__saved-avatar"
                    style={{
  ...
  function sendLogin() {
    loginUser(
      formFields.find((field) => field.id === "login").value,
      formFields.find((field) => field.id === "password").value,
      formFields.find((field) => field.id === "remember").value,
    );
  }
}
...
const mapDispatchToProps = (dispatch) => ({
  loginUser: (login, password, remember) =>
