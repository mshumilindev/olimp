...
function AdminCourses({
  ...
  if (params) {
    if (
      ...
      list &&
      list.length &&
      !list.find((item) => item.id === params.subjectID)
    ) {
    ...
    if (
      ...
      list.length &&
      list.find((item) => item.id === params.subjectID) &&
      list.find((item) => item.id === params.subjectID).coursesList &&
      !list
        .find((item) => item.id === params.subjectID)
        .coursesList.find((item) => item.id === params.courseID)
    ) {
      navigate("/admin-courses/" + params.subjectID);
    }
    if (
      params.moduleID &&
      list &&
      ...
      list
        .find((item) => item.id === params.subjectID)
        .coursesList.find((item) => item.id === params.courseID).modules &&
      !list
        .find((item) => item.id === params.subjectID)
        .coursesList.find((item) => item.id === params.courseID)
  ...
  const [subjectFields, setSubjectFields] = useState(
    JSON.stringify(getSubjectFields(getSubjectModel())),
  );
  ...
  const handleCreateSubject = useCallback(() => {
    ...
    newSubject.name = {
      en: newSubjectFields
        .find((item) => item.id === "subjectName_en")
        .value.replace(/\s*$/, ""),
      ru: newSubjectFields
        .find((item) => item.id === "subjectName_ru")
        .value.replace(/\s*$/, ""),
      ua: newSubjectFields
        .find((item) => item.id === "subjectName_ua")
        .value.replace(/\s*$/, ""),
    };
    newSubject.id = identify(transliterize(newSubject.name["ua"]));

    if (
      list.some((item) => item.id.toLowerCase() === newSubject.id.toLowerCase())
    ) {
  ...
  const myList = useMemo(() => {
    return filteredList?.filter((item) => {
      const coursesList = allCoursesList?.filter(
        (allItem) => allItem.id === item.id,
      )?.[0]?.coursesList;
      const onlyMyCourses = coursesList?.filter(
  ...
  const getCoursesQty = useCallback(
    (subjectID) => {
      const coursesList = allCoursesList?.find(
        (allItem) => allItem.id === subjectID,
      )?.coursesList;
  ...
  const otherList = useMemo(() => {
    return filteredList?.filter(
      (item) =>
        !allCoursesList
          ?.find((allItem) => allItem.id === item.id)
          ?.coursesList?.some(({ teacher }) => teacher === user?.id),
    );
  }, [filteredList, user, allCoursesList]);

  const showCreateSubjectModal = useCallback(
    (e) => {
      e.preventDefault();
    ...
    <div className="adminCourses">
        ...
        <div className="section__title-holder">
          ...
          <div className="section__title-actions">
            <a
              ...
              onClick={showCreateSubjectModal}
            >
        ...
        <div className="adminLibrary__list widget">
          {
            <>
              <div className="widget__descr">
                ...
                <p>
                  <strong>
                    Перед створенням нового основного предмету запевніться, що
                    його ще не існує
                  </strong>
                </p>
              </div>
                  ...
                  <>
                    ...
                    {myList.map((subject) => (
                      <AdminCoursesSubject
                        ...
                        coursesQty={getCoursesQty(subject.id)}
                        params={params}
                        loading={loading}
                        subject={subject}
                        key={subject.id}
                  ...
                  <>
                    ...
                    {otherList.map((subject) => (
                      <AdminCoursesSubject
                        ...
                        setIsLessonCoppied={setIsLessonCoppied}
                        coursesQty={
                          user?.role === "admin" ? subject.children : 0
                        }
                      />
                    ))}
                  </>
      ...
      {showModal ? (
        <Modal onHideModal={() => toggleModal()}>
          <Form
            loading={loading}
            heading={translate("create_subject")}
            fields={JSON.parse(subjectFields)}
            setFieldValue={setFieldValue}
            formAction={handleCreateSubject}
            formError={formError}
