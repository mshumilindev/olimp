...
function StudentCourses({ classData, allCoursesList }) {
  ...
  function _renderCourse(item) {
    ...
    return (
      <div
        className="block studentCourses__list-item"
        key={item.course.id}
        id={item.course.id}
      >
        <h2 className="block__heading">
          <Link to={"/courses/" + item.subject.id + "/" + item.course.id}>
            <span className="block__heading-subheading">
              {item.subject.name[lang]
                ? item.subject.name[lang]
                : item.subject.name["ua"]}
            </span>
            {item.course.name[lang]
              ? item.course.name[lang]
              : item.course.name["ua"]}
          </Link>
        </h2>
      ...
      <div className={classNames("alphabetIndex", { show: showIndex })}>
        ...
        <div className="alphabetIndex__list">
          {filterCourses().map((item) => {
            ...
            return (
              <div
                className="alphabetIndex__item"
                key={item.course.id}
                onClick={() => scrollTo(item.course.id)}
              >
                {item.course.name[lang]
                  ? item.course.name[lang]
                  : item.course.name["ua"]}
