...
import {
  fetchSiteSettings,
  updateSiteSettings,
} from "../../redux/actions/siteSettingsActions";
...
function AdminSettings({
  ...
  if (
    ...
    JSON.stringify(contactList) !== initialContacts
  ) {
  ...
  if (
    ...
    JSON.stringify(siteSettingsList) !== initialSiteSettings
  ) {
  ...
  const removeBlock = useCallback(
    (blockID) => {
      ...
      newList.splice(
        newList.indexOf(newList.find((item) => item.id === blockID)),
        1,
      );
  ...
  const updateModel = useCallback(
    (fieldID, value) => {
      ...
      if (
        fieldID.includes("_nameUA") ||
        fieldID.includes("_nameRU") ||
        fieldID.includes("_nameEN") ||
        fieldID.includes("_tel")
      ) {
        setContacts(JSON.stringify(newList));
      } else {
        ...
        <div className="grid">
          ...
          <div className="grid_col col-6 mapContainer">
            <div className="widget">
              ...
              {contactList && loading ? <Preloader /> : null}
            </div>
          </div>
        </div>
      </section>
...
const mapDispatchToProps = (dispatch) => ({
  ...
  updateSiteSettings: (siteSettings) =>
    dispatch(updateSiteSettings(siteSettings)),
  setUpdates: (type) => dispatch(setUpdates(type)),
});
export default connect(
  mapStateToProps,
  mapDispatchToProps,
)(withFilters(AdminSettings, true));
